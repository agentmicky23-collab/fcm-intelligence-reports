...
function CompareTab() {
  ...
  const metrics = [
    ...
    { label: 'FCM Score', key: 'score', format: (v) => v.toString() },
    { label: 'Valuation Multiple', key: null, format: null, compute: (l) => l.fees ? `${getValuationMultiple(l.price, l.fees)}x` : '—' },
    { label: 'Region', key: 'region', format: (v) => v },
    { label: 'Tenure', key: 'tenure', format: (v) => v ? v.charAt(0).toUpperCase() + v.slice(1) : '—' },
    { label: 'Revenue Health', key: 'health', format: null, isDot: true },
    { label: 'Location Strength', key: 'locationStrength', format: null, isDot: true },
    { label: 'Risk Level', key: 'risk', format: null, isDot: true },
  ];

  // Find best values for highlighting
  const bestValues = {};
  if (selectedListings.length > 1) {
    bestValues.score = Math.max(...selectedListings.map(l => l.score));
    bestValues.fees = Math.max(...selectedListings.map(l => l.fees));
    const withFees = selectedListings.filter(l => l.fees > 0);
    if (withFees.length) {
      bestValues.valuationId = withFees.reduce((best, l) => (l.price / l.fees) < (best.price / best.fees) ? l : best, withFees[0]).id;
    }
  }
    ...
    <div>
      ...
      <div style={{ marginBottom: '24px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
          ...
          {selected.length > 0 && (
            <button onClick={() => setSelected([])} style={{ fontSize: '13px', color: '#D4AF37', background: 'none', border: 'none', cursor: 'pointer' }}>Clear all</button>
          )}
        </div>
            ...
            <tbody>
              {metrics.map((metric, mi) => (
                <tr key={mi}>
                  ...
                  {selectedListings.map(l => {
                    let value, isBest = false;
                    if (metric.compute) {
                      value = metric.compute(l);
                      if (metric.label === 'Valuation Multiple' && bestValues.valuationId === l.id) isBest = true;
                    } else if (metric.isDot) {
                      const dotVal = l[metric.key];
                      value = (
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}>
                          <div style={{ width: '10px', height: '10px', borderRadius: '50%', background: getHealthDot(dotVal) }} />
                          <span style={{ textTransform: 'capitalize' }}>{dotVal}</span>
                        </div>
                      );
                    } else {
                      value = metric.format ? metric.format(l[metric.key]) : l[metric.key];
                      if (metric.key === 'score' && l.score === bestValues.score) isBest = true;
                      if (metric.key === 'fees' && l.fees === bestValues.fees && l.fees > 0) isBest = true;
                    }
                    return (
                      <td key={l.id} style={{
                        padding: '12px 16px', borderBottom: '1px solid #1e2733', textAlign: 'center',
                        fontSize: '14px', fontFamily: metric.key === 'price' || metric.key === 'fees' || metric.key === 'score' ? 'JetBrains Mono, monospace' : 'inherit',
